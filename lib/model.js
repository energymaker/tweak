...
export async function ask(model, prompt, schema, signal) {
  signal = withTimeout(signal);
  const [provider, ...rest] = String(model).split(':');
  const name = rest.join(':');
  if (provider === 'ollama') {
    const body = { model: name, messages: [{ role: 'user', content: prompt }], stream: false, format: schema || 'json', options: { temperature: 0.2, num_ctx: Number(process.env.TWEAK_NUM_CTX || 8192), num_predict: Number(process.env.TWEAK_MAX_OUTPUT || 1400) } };
