...
export async function ask(model, prompt, schema, signal, maxOutput, temperature = 0.2) {
  signal = withTimeout(signal);
  const [provider, ...rest] = String(model).split(':');
  const name = rest.join(':');
  if (provider === 'ollama') {
    const body = { model: name, messages: [{ role: 'user', content: prompt }], stream: false, format: schema || 'json', options: { temperature, num_ctx: Number(process.env.TWEAK_NUM_CTX || 8192), num_predict: maxOutput || Number(process.env.TWEAK_MAX_OUTPUT || 1400) } };
  ...
  if (provider === 'api') {
    ...
    const r = await fetch(API_BASE() + '/chat/completions', { method: 'POST', signal, headers, body: JSON.stringify({ model: name, temperature, messages: [{ role: 'user', content: prompt }] }) });
    if (!r.ok) {
