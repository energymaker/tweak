...
class Page {
  constructor(win) {
    this.win = win; this.wc = win.webContents;
    // Every main-document load since the last goto, with its HTTP status, so a
    // wall that flashed past (a 429, a sign-in redirect) is still known.
    this.seen = []; this.status = null;
    this.wc.on('did-navigate', (_e, url, status) => { this.status = status || null; this.seen.push({ url, status: this.status }); });
  }
...
// Is the page in front of us the one that was asked for? null when it is,
// otherwise { kind, reason, finalUrl } (see walls.js).
export async function pageWall(page, requested) {
  const final = page.url();
  const first = wallFor({ requested, final, status: page.status });
  if (first) return first;
  return wallFor({ requested, final, status: page.status, dialog: await consentWall(page) });
}

// When loading failed, a wall seen on the way explains it better than the error.
export function wallDuringLoad(page, requested) {
  for (const nav of [...(page.seen || [])].reverse()) {
    const w = wallFor({ requested, final: nav.url, status: nav.status });
    if (w && w.kind !== 'moved') return w;
  }
  return null;
}

// A consent page or a consent dialog over the page, or ''.
export async function consentWall(page) {
  ...
  return page.evaluate(() => {
    ...
    // A dialog over the middle of the page that talks about cookies or consent.
    // Only a box that covers most of the screen counts, so a cookie bar along
    // the bottom never stops a run.
    const words = /cookie|consent|before you continue|accept all|reject all|privacy settings/i;
    const vw = innerWidth, vh = innerHeight;
    const cover = el => { const r = el.getBoundingClientRect(); return r.width * r.height >= 0.3 * vw * vh; };
    for (const [x, y] of [[0.5, 0.5], [0.5, 0.3], [0.5, 0.7]]) {
      for (let el = document.elementFromPoint(vw * x, vh * y); el && el !== document.body && el !== document.documentElement; el = el.parentElement) {
        const cs = getComputedStyle(el);
        const modal = el.matches('dialog[open], [role=dialog], [role=alertdialog], [aria-modal=true]') || cs.position === 'fixed';
        if (!modal || !cover(el)) continue;
        const said = el.tagName === 'IFRAME' ? (el.id + ' ' + el.title + ' ' + el.src) : (el.innerText || '').slice(0, 3000);
        if (words.test(said)) return 'a cookie or consent dialog covers the page';
      }
    }
    return '';
  });
