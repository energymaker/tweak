...
async function runProject({ request, project, model, fallback = '', maxAttempts = 5, onEvent = () => {}, signal }) {
  ...
  let feedback = '', applied = null, names = null, retriedBig = false;
  ...
  for (let n = 1; n <= maxAttempts; n++) {
    ...
    try {
      ...
      if (/valid JSON|returned nothing/.test(e.message)) { feedback = 'Your reply was not valid JSON. Reply with only the JSON object.'; continue; }
    ...
    if (bad) { attempt.problem = bad; feedback = bad; emit('attempt', { attempt }); continue; }
...
export async function runTweak({ target = 'web', request, url, model, fallback = '', headless = true, maxAttempts = 3, onEvent = () => {}, signal }) {
  ...
  let baselineBrowser, testBrowser;
  ...
  const finish = async (status, reason) => {
    ...
    await fs.mkdir(HOME, { recursive: true });
    let line;
    try { line = JSON.stringify({ kind: 'run', id, time: new Date().toISOString(), request, url, model, status, reason, seconds: result.seconds, attempts: result.attempts.map(a => ({ n: a.n, verdict: a.verdict, problem: a.problem, tokens: a.tokens })) }) + '\n'; }
    catch (e) { line = '{}\n'; }
  ...
  for (let n = 1; n <= maxAttempts; n++) {
    if (signal && signal.aborted) return finish('stopped', 'Stopped.');
    if (fallback && fallback !== model && n === 3) {
      model = fallback;
      emit('step', { key: 'escalate', label: `The small model could not do it. Trying ${fallback.replace(/^(ollama|api):/, '')}` });
    }
    const attempt = { n, verdict: 'fail', problem: '', checks: [], tokens: null, model };
    ...
    emit('step', { key: 'write', label: n === 1 ? 'Writing the tweak' : `Fixing it, try ${n} of ${maxAttempts}` });
    result.model = model;
    let gen;
    try {
      const r = await ask(model, buildPrompt({ url, request, pageOutline, feedback, known }), SCHEMA, signal);
      gen = r.json; attempt.tokens = r.tokens;
    } catch (e) {
      ...
      attempt.problem = e.message;
      emit('attempt', { attempt });
      if (/valid JSON|returned nothing/.test(e.message) && n < maxAttempts) { feedback = 'Your reply was not valid JSON. Reply with only the JSON object.'; continue; }
      return finish('fail', 'The model could not be used: ' + e.message);
    }
    ...
    gen.checks = mendChecks(gen.checks);
    const fromCss = !gen.js.trim() ? checksFromCss(gen.css) : [];
    const fromJs = gen.js.trim() ? checksFromJs(gen.js) : [];
    ...
    if (attempt.verdict === 'works') {
      await rememberWorking({ url, request, gen, checks: attempt.checks }).catch(() => {});
      result.best = n; return finish('works', 'The change was tested on the real page and the checks prove it.'); }
