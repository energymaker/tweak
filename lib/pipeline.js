...
import { VIEWPORT, launchTestBrowser, goto, outline, runChecks, tweakError, pageWall, wallDuringLoad, freshProfile, dropProfile, loadCookies, saveCookies, isResponsive } from './browser.js';
...
// For benchmarks only: allRounds keeps running every round after a pass, and
// remember: false keeps passes out of what-worked.json, so no task learns from
// an earlier one.
export async function runTweak({ target = 'web', request, url, model, fallback = '', headless = true, maxAttempts = 3, attempts = 1, escalate = true, allRounds = false, remember = true, onEvent = () => {}, signal }) {
  ...
  const finish = async (status, reason) => {
    ...
    await fs.mkdir(HOME, { recursive: true });
    let line;
    try { line = JSON.stringify({ kind: 'run', id, time: new Date().toISOString(), request, url, model, status, reason, wall: result.wall || null, seconds: result.seconds, modelCalls, attempts: result.attempts.map(a => ({
  ...
  // A page that is not the one asked for stops the run as 'walled': never a
  // pass or a fail, because the model never saw the real page.
  const walled = wall => {
    result.wall = wall;
    const why = wall.kind === 'consent'
      ? `Cannot test this: ${wall.reason}, so anything built now would be about the wall, not the real page. Click "Open test browser", accept or reject the cookie page once, close that window, then try again.`
      : wall.kind === 'moved'
        ? `Cannot test this: ${wall.reason}, so anything built now would be for that site, not the one you asked for.`
        : `Cannot test this: ${wall.reason}, so anything built now would be about that page, not the one you asked for.`;
    return finish('walled', why);
  };
  ...
  try {
    ...
    const wall = await withLimit(pageWall(basePage, url), 'Checking the page', 20000);
    if (wall) return walled(wall);
    ...
    const wall = basePage && wallDuringLoad(basePage, url);
    if (wall) return walled(wall);
  ...
  for (let round = 1; round <= maxAttempts; round++) {
    ...
    for (let k = 1; k <= samples; k++) {
      ...
      try {
        ...
        if (attempt.verdict === 'works') {
          if (!winner) { winner = n; if (remember) await rememberWorking({ url, request, gen, checks: attempt.checks }).catch(() => {}); }
          continue;
        }
    ...
    if (winner && !result.best) result.best = winner;
    if (result.best && !allRounds) return finish('works', 'The change was tested on the real page and the checks prove it.');
    roundFeedback = nextFeedback;
  }
  if (result.best) return finish('works', 'The change was tested on the real page and the checks prove it.');
