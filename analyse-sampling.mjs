...
// The two tables, in Markdown: N=1, 3 and 5 side by side, then per task.
export function report(runs) {
  ...
  return [
    '| N | Passed | Not proven | Incomplete | Model calls | Calls per task | Passes relying on a model-written check |',
    '|---|---|---|---|---|---|---|',
    ...rows.map(s => `| ${s.n} | ${s.passed} of ${s.tasks} | ${s.unproven} | ${s.incomplete} | ${s.calls} | ${s.callsPerTask.toFixed(1)} | ${s.reliedOnModelChecks} |`),
    '',
    'Per task (round.sample of the winning attempt, or the outcome):',
    '',
    `| Task | ${rows.map(s => 'N=' + s.n).join(' | ')} |`,
    `|---|${rows.map(() => '---').join('|')}|`,
    ...runs.map((run, i) => `| ${String(run.request).replace(/\|/g, '/')} | ${rows.map(s => { const o = s.perTask[i]; return o.status === 'works' ? `works ${o.round}.${o.sample}${o.reliedOnModelChecks ? ' (model check)' : ''}` : o.status; }).join(' | ')} |`)
  ].join('\n') + '\n';
}

// A model's part of the bench report when --all-rounds forced every round.
export function forcedSection(model, attempts, runs) {
  return `## ${model}\n\n**All rounds were forced (--all-rounds).** Every task ran all 3 rounds of ${attempts} samples, even after a pass, so the time and number of tries of each run say nothing about a normal run. The table below is what N=1, 3 and 5 would have done, worked out from the attempts that ran, using only samples 1 to N of each round and stopping at the first round with a pass.\n\n` + report(runs) + '\n';
}
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  console.log(`${runs.length} runs from ${file}\n`);
